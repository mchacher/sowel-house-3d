---
name: house3d-feature
description: |
  Implements a feature or a phase in sowel-house-3d. Use when:
...
# sowel-house-3d — feature workflow
...
1. Plan and mapping types, validation (pure, tested)
2. Sowel client: REST bootstrap, WebSocket subscription, order calls
3. Scene: procedural geometry from the plan, materials, lighting
4. State binding: Sowel readings → scene, clicks → orders
5. HUD (React) and mobile
6. Tests for every pure module
...
```bash
npm run validate     # lint, typecheck, format:check, test, build
```
...
Conventional commits, scopes: plan, scene, client, hud, ui, ci. Tick acceptance criteria in `spec.md` and tasks in `plan.md`.
